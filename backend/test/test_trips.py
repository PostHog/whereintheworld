...
class TestTrips(BaseTest):
    def test_trip_start_must_always_be_before_end(self):
        ...
        with self.assertRaises(ValidationError) as e:
            Trip.objects.create(user=self.user, start=dt.date(2020, 4, 28), end=dt.date(2020, 4, 27), city=self.paris)
    ...
    def test_one_day_trips_are_allowed(self):
        ...
        Trip.objects.create(user=self.user, start=dt.date(2020, 2, 27), end=dt.date(2020, 2, 27), city=self.london)
