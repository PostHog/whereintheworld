...
app.post(`/trip`, async (req, res) => {
    ...
    for (let scheduledTrip of scheduledTrips) {
        ...
        if (
            ...
            (scheduledTrip.start <= startDate && endDate <= scheduledTrip.end) ||
            // |~~~~~SCHEDULED TRIP~~~~|xxx|-----NEW TRIP-----|
            (scheduledTrip.end > startDate && scheduledTrip.start < startDate ) ||
            // |-----NEW TRIP-----|xxx|~~~~~SCHEDULED TRIP~~~~|
            (scheduledTrip.start < endDate && scheduledTrip.end > endDate)
        ) {
